...
- Active academy chain catalogue
- Chain and item unlock levels

Orders must never request an item that the current Producer cannot realistically produce.

Football Academy Orders draw from all four active chains. New Order sets must
represent at least two chains, and replacement Orders use recent and currently
visible chain counts to avoid repetitive runs. Orders may contain one or two
items; two-item Orders may combine different chains.

Existing valid saved Orders remain until completion. Their replacements are
generated by the current multi-chain engine.
